...
note: nodes are inaccessable in Java's linkedList implementation, so identity hashcode of the objects contained in the nodes are used instead
currently returns the indexes of the nodes for the intersect and the repeating values.
checking by node would require an custom linkedlist implementation